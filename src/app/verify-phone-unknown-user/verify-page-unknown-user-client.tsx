...
export default function VerifyPhoneUnknownUser({ temp_phone }: { temp_phone: string | undefined }) {
  ...
  useEffect(() => {
    ...
    window.phoneEmailListener = async (userObj: any) => {
      ...
      if (!userObj?.user_json_url) {
        ...
        setTimeout(() => router.replace(`/search?crnm=${encodeURIComponent(carNumber)}`), 2000)
        return
      }
      ...
      try {
        ...
        setTimeout(() => router.replace(`/search?crnm=${encodeURIComponent(carNumber)}`), 1500)
